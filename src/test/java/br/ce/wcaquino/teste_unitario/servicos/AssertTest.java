...
public class AssertTest {
	...
	public void test() {
		...
		Assert.assertEquals(Math.PI, 3.14, 0.01);	// como não quero precisar colocar o valor com total precisão, é feito a declaração do delta;
...
//		Assert.assertEquals(i, i2);	// não compila, pois é preciso usar tipos compatíveis, primitivo com primitivo, ou o contrário;
		...
		Assert.assertEquals("Bola".toLowerCase(), "bola");	// é preciso comparar usando o valor em caixa baixa com caixa baixa
		Assert.assertNotEquals("Bola", "Casa");
		Assert.assertTrue("Bola".startsWith("Bo"));
		
		// no caso de objetos de outras classes, quem deve dizer se eles são iguais é a própria classe usando o método equals
